...
Kirby::plugin('afbora/kirby-template-hooks', [
    'hooks' => [
        ...
        'page.changeStatus:before' => function ($page, $status, $position = null) {
            try {
                $template = $page->intendedTemplate();
                kirby()->trigger("page.{$template}.changeStatus:before", ['page'=> $page, $status, 'position'=> $position]);
            }
            catch (Exception $e) {
                throw $e;
            }
        ...
        'page.update:before' => function ($page, $values, $strings) {
            try {
                $template = $page->intendedTemplate();
                kirby()->trigger("page.{$template}.update:before", ['page'=> $page, $values, 'strings'=> $strings]);
            }
            catch (Exception $e) {
                throw $e;
            }
